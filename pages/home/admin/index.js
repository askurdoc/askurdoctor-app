...
import { notVerified } from '../../../services/qualification.service';
...
export default function Dashboard(props) {
  ...
  const init = async () => {
    ...
    try {
      const response = await notVerified(1);
      const analytics = await getAnalytics();
      Notiflix.Loading.remove();
      setIsLoading(false);
      if (response.data && response.data.response) {
        setNonVerified(response.data.response);
      } else {
      ...
      if (analytics.data && analytics.data.response) {
        setAnalytics(analytics.data.response);
      }
    } catch (e) {
      setIsLoading(false);
      Notiflix.Loading.remove();
    ...
    return nonVerified.map((item, i) => {
      ...
      return (
        <tr>
          ...
          <td>
            <p className="mb-4">{' '}</p>
            <h5 className="my-4 text-primary">
            Doctor
            </h5>
          </td>
          <td>
            <div className="d-flex align-items-center">
              <div>
                ...
                <br />
                <Link href={`/profile/doctors/${item.userId}`}>
                  <a className="mt-5 pt-5 mb-1 text-success">
                    View and Verify
                  </a>
                </Link>
              </div>
                ...
                <div className="col-xl-12 col-xxl-12 col-lg-12 col-md-12">
                  <div className="card">
                    <div className="card-header border-0 pb-0">
                      <h4 className="card-title">Waiting for your approval</h4>
                    </div>
